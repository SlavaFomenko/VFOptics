...
app.post('/registration',(req,res)=>{
    ...
    const sqlQuery = `insert into Customers (customer_id, first_name, last_name, password, tel_number, role) 
    values ('${login}','${first_name}','${last_name}','${password}','${tel_number}','${role}')`
    console.log(sqlQuery)

    connection.query(sqlQuery,(err, result)=>{
        if(err){
            console.log(err)
            res.json({ status: 'err', message: 'Користувач з таким логіном або номером телефону вже існує' });
            return
        }
        res.json({ status: 'success', message: 'The user has been added to the database' });
